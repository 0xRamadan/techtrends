...
def metrics():
    ...
    #connections = connection.execute('SELECT COUNT(*) FROM connections')
    response = app.response_class(
        response=json.dumps({"status": "success", "code": 200, "data": {
                            "db_connection_count": db_connection_count, "post_count": posts}}),
        status=200,
        mimetype='application/json'
    )
...
def post(post_id):
    ...
    if post is None:
        app.logger.info(f'No article with {post_id} Id was found')
